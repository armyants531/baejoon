...
// 좌표 평면에서 거리의 대소관계를 비교할 수 있는 함수 만듦
ll distance(coord A, coord B) {
	ll len = (A.x - B.x) * (A.x - B.x) + (A.y - B.y) * (A.y - B.y);
...
int main() {
	...
	if (sign1 == 0 && sign2 == 0) { // 네 점이 일직선 상에 있거나 평행하지 않은 두 선분의 한 끝이 붙은 경우(ex: 'ㄱ'자 모양)
		ll lenAB = distance(A, B);
		ll lenAC = distance(A, C);
		ll lenAD = distance(A, D);
		ll lenBC = distance(B, C);
		ll lenBD = distance(B, D);
		if ((((lenAB < lenAC)) && (lenAB < lenAD)) || (((lenAB < lenBC)) && (lenAB < lenBD))) // 'A와 B의 거리 < A와 C의 거리'이고 'A와 B의 거리 < A와 D의 거리'또는
		...
		if ((sign1 < 0 && sign2 < 0) || (sign1 == 0 && sign2 < 0) || (sign1 < 0 && sign2 == 0)) { // 어느 세 점도 일직선 상에 있지 않고, 교차하는 경우 // 한 점을 제외한 세 점이 일직선 상에 있고 교차하는 경우(이 때 세 점은 서로 겹치지 않음)
			cout << 1 << "\n";

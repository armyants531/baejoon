// 맹독 방벽  // 기하학  // Graham's scan  // CCW
// 반지름 L인 원 둘레 + 볼록 껍질 둘레
#include <bits/stdc++.h>
...
int main() {
	...
	double len = 0;
	// 하나씩 뽑아 길이 계산 후 둘레 길이에 더해줌
	int first = st.top();
	while (st.size() > 1) {
		int a = st.top();
		st.pop();
		int b = st.top();
		len += sqrt((points[a].x - points[b].x) * (points[a].x - points[b].x) + (points[a].y - points[b].y) * (points[a].y - points[b].y));
	}
	int last = st.top();
	len += sqrt((points[first].x - points[last].x) * (points[first].x - points[last].x) + (points[first].y - points[last].y) * (points[first].y - points[last].y));
	double ans = 2 * M_PI * L + len;
	cout << round(ans) << "\n";
